...
(async () => {
  try {
    ...
    for (const framework of ['react', 'vue']) {
      ...
      assert.equal(await indicator.evaluate(el => getComputedStyle(el, '::after').height), '2px', 'selected underline remains separate from hover');
      assert.equal(await list.locator('[data-tab-indicator]').evaluate(el => getComputedStyle(el.previousElementSibling).backgroundColor), 'rgb(208, 196, 228)', 'selection fill travels with the underline');
