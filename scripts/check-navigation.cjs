...
(async () => {
  try {
    ...
    for (const framework of ['react', 'vue']) {
      ...
      const surface = nav.locator('[data-hover-highlight]');
      const overview = nav.getByRole('link', { name: '概要', exact: true });
      const projects = nav.getByRole('button', { name: 'プロジェクト', exact: true });
      const website = nav.getByRole('link', { name: 'Web サイト', exact: true });
      const aligned = async target => {
        await page.waitForTimeout(360);
        const actual = await surface.boundingBox();
        const expected = await target.boundingBox();
        for (const key of ['x', 'y', 'width', 'height']) assert(Math.abs(actual[key] - expected[key]) < 1, `${framework}: highlight ${key} aligns with ${await target.innerText()}`);
      };
      // Entry from either side grows from the pointer, using the same motion as Dropdown.
      await page.waitForTimeout(1000);
      for (const edge of ['left', 'right']) {
        const box = await overview.boundingBox();
        const x = edge === 'left' ? box.x + 2 : box.x + box.width - 2;
        const y = box.y + box.height / 2;
        await page.mouse.move(edge === 'left' ? box.x - 12 : box.x + box.width + 12, y);
        await page.waitForTimeout(160);
        await page.mouse.move(x, y);
        await page.waitForTimeout(50);
        const entering = await surface.boundingBox();
        assert(entering.width > 0 && entering.width < box.width, 'background grows from the entry point');
        if (edge === 'right') assert(entering.x > box.x);
        await aligned(overview);
      }
      await projects.hover();
      await page.waitForTimeout(50);
      const moving = await surface.boundingBox();
      assert(moving.y > (await overview.boundingBox()).y && moving.y < (await projects.boundingBox()).y, 'one surface travels between rows');
      await aligned(projects);
      assert.equal(await projects.locator('span').first().evaluate(el => getComputedStyle(el).backgroundColor), 'rgba(0, 0, 0, 0)', 'the trigger does not paint a second hover background');
      await website.hover();
      await aligned(website);
      assert.equal(await surface.count(), 1, 'nested links and headings share the same surface');
      ...
      await page.mouse.move(600, 600);
      await aligned(overview);
      assert.equal(await surface.evaluate(el => getComputedStyle(el).opacity), '1', 'mouse leave restores the keyboard focus highlight');
      await overview.evaluate(el => el.blur());
      ...
      // Collapsing content cannot leave an old surface extending the scroll area.
      await website.hover();
      await aligned(website);
      await projects.evaluate(el => el.click());
      await page.waitForTimeout(360);
      assert(await nav.evaluate(el => el.scrollHeight <= el.clientHeight));
      await projects.click();
      await projects.evaluate(el => { el.disabled = true; });
      await overview.hover();
      await projects.hover({ force: true });
      await page.waitForTimeout(180);
      assert.equal(await surface.evaluate(el => getComputedStyle(el).opacity), '0', 'disabled headings do not highlight');
      await projects.evaluate(el => { el.disabled = false; });
      ...
      await page.waitForTimeout(180);
      assert.equal(await surface.evaluate(el => getComputedStyle(el).opacity), '0', 'disabled links do not highlight');
      ...
      await aligned(nav.locator('[data-hover-item]:hover'));
      await page.mouse.move(600, 600);
      await nav.getByRole('link').last().focus();
      await aligned(nav.getByRole('link').last());
