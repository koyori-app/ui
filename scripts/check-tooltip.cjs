...
(async () => {
  try {
    ...
    for (const framework of ['react', 'vue']) {
      ...
      await story('dynamic');
      await trigger.focus(); await visible();
      assert.equal(await trigger.getAttribute('aria-describedby'), 'existing-description changing-help');
      // Only the child renders; focus and hover never leave or re-enter the trigger.
      await trigger.press('Space');
      assert.equal(await trigger.getAttribute('aria-describedby'), 'updated-description changing-help', `${framework}: child updates retain the tooltip description`);
      assert(await trigger.evaluate(el => el === document.activeElement));
      await visible();
      const updatedCdp = await page.context().newCDPSession(page);
      const { root: updatedRoot } = await updatedCdp.send('DOM.getDocument');
      const { nodeId: updatedId } = await updatedCdp.send('DOM.querySelector', { nodeId: updatedRoot.nodeId, selector: '#storybook-root button' });
      const { nodes: updatedNodes } = await updatedCdp.send('Accessibility.getPartialAXTree', { nodeId: updatedId, fetchRelatives: false });
      assert.equal(updatedNodes[0].description.value, '操作を変更しました。 最初の説明');
      await updatedCdp.detach();
      await trigger.press('Space');
      assert.equal(await trigger.getAttribute('aria-describedby'), 'changing-help', 'removing the child description preserves the tooltip');
      await trigger.press('Space');
      assert.equal(await trigger.getAttribute('aria-describedby'), 'existing-description changing-help');
      await page.getByRole('button', { name: '内容を変更' }).click();
      await trigger.focus(); await visible();
      assert.equal(await tooltip.innerText(), '変更した説明');
      await page.getByRole('button', { name: '無効を切り替え' }).dispatchEvent('click');
      assert.equal(await trigger.getAttribute('aria-describedby'), 'existing-description', 'disabling removes only the owned description token');
      await trigger.focus(); await hidden();
      await trigger.press('Space');
      assert.equal(await trigger.getAttribute('aria-describedby'), 'updated-description', 'child updates remain untouched while disabled');
      await page.getByRole('button', { name: '無効を切り替え' }).click();
      await trigger.focus(); await visible();
      assert.equal(await trigger.getAttribute('aria-describedby'), 'updated-description changing-help');
      ...
      const removedTrigger = await trigger.elementHandle();
      await page.getByRole('button', { name: 'Tooltipを破棄' }).dispatchEvent('click');
      await tooltip.waitFor({ state: 'detached' });
      assert.equal(await removedTrigger.getAttribute('aria-describedby'), 'updated-description', 'destroy removes only the owned token');
      await removedTrigger.evaluate(el => el.setAttribute('aria-describedby', 'after-unmount'));
      assert.equal(await removedTrigger.getAttribute('aria-describedby'), 'after-unmount', 'destroy stops observing the trigger');
      await removedTrigger.dispose();
